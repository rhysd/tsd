...
function gitFromPathname (pathname: string) {
  ...
  if (segments.length === 0) {
    path = TSD_FILE
  } else if (!isDefinition(path) && segments[segments.length - 1] !== TSD_FILE) {
    path += `/${TSD_FILE}`
  }
...
/**
 * Parse the TSD dependency field.
 */
export default function parseDependency (raw: string): Dependency {
  ...
  if (protocol === 'file:') {
    ...
    invariant(
      filename === TSD_FILE || isDefinition(filename),
      `Only ".d.ts" files and "${TSD_FILE}" are supported`
    )
